def call(env, buildNotice, buildResults, errorContent, STOP_SECKILL, TKE_DC, resourcesArray, POD_REPLICAS) {
    ...
    if (buildNotice.type == 'dingding') {
        ...
        if (STOP_SECKILL == 'false') {
            content = "# ${env.JOB_NAME} [构建${buildResults}](${env.BUILD_URL}console)\n________________\n> 项目名称：${env.JOB_NAME}\n\n> 构建编号：第${env.BUILD_NUMBER}次构建\n\n> 触发原因：${USER}\n\n> 节点数量：${TKE_DC}\n\n> 资源对象：${resourcesArray.name}\n\n> Pod副本：${POD_REPLICAS}\n\n> 构建时长：${buildTime}\n\n"
        }else {
            content = "# ${env.JOB_NAME} [构建${buildResults}](${env.BUILD_URL}console)\n________________\n> 项目名称：${env.JOB_NAME}\n\n> 构建编号：第${env.BUILD_NUMBER}次构建\n\n> 触发原因：${USER}\n\n> 构建时长：${buildTime}\n\n"
        }
